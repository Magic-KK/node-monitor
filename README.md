...
![Version](https://img.shields.io/badge/version-v1.29.0-6C63FF.svg?style=flat-square)
...
**v1.29.0 新增** - 支持配置热更新，修改配置后无需重启服务即可生效！

#### 如何使用

1. **通过配置管理 UI 修改**
   - 点击界面右上角 ⚙️ 配置管理 按钮
   - 在 基础设置 标签页修改检查间隔、超时时间等
   - 点击 💾 保存设置，配置立即生效
...
3. **查看配置变更历史**
   - 在配置管理弹窗中切换到 📜 变更历史 标签页
   - 查看所有配置变更记录（时间、类型、变更内容）
   - 支持清除历史记录

#### 热更新支持项

- ✅ 检查间隔（checkInterval）- 修改后立即重启健康检查调度器
- ✅ 超时时间（timeout）- 下次检查时生效
- ✅ 团队名称（teamName）- 界面立即更新
- ✅ 工作空间（workspace）- 下次检查时生效
- ✅ OpenClaw 配置重载 - 重新读取 openclaw.json

#### WebSocket 实时通知

配置变更后，所有连接的客户端会通过 WebSocket 收到实时通知，界面自动更新。
...
### v1.29.0 (2026-03-05) - 配置热更新增强版 🔥
- 🔥 **新增** 配置热更新系统，修改配置无需重启服务
- ⚙️ **实现** `restartHealthCheckScheduler()` 动态重启健康检查调度器
- 📜 **新增** 配置变更历史记录（保留最近 50 条）
- 🔌 **新增** API 端点：
  - `GET /api/config/history` - 获取配置变更历史
  - `POST /api/config/history/clear` - 清除配置历史
- 🎨 **新增** 配置变更历史 UI 标签页（变更历史/热更新状态）
- 🔄 **WebSocket 实时通知** - 配置变更自动推送给所有客户端
- 📊 **增强** `/api/settings` 和 `/api/reload-config` 支持热更新
- 📝 **完善** README 文档，添加热更新使用说明

### v1.28.0 (2026-03-05) - HTTPS 安全增强版 🔒
